...
<div class="wrap">
        ...
        <form method="GET" action="{{ route('admin.transactions') }}" class="p-4">
            <div style="display: flex; gap: 12px; align-items: end; flex-wrap: wrap;">
                ...
                @if(request('search') || request('type') || request('status'))
                    <a href="{{ route('admin.transactions') }}" style="height: 40px; padding: 0 20px; border-radius: 10px; background: #2563EB; color: white; font-size: 12px; font-weight: 900; cursor: pointer; display: inline-flex; align-items: center; text-decoration: none;">
                        Clear
                    </a>
                @endif
            </div>
    ...
    <div class="whitePanel">
        ...
        @endif
        @if(session('error'))
            <div style="margin: 12px 14px; padding: 12px; background: #dbeafe; border: 1px solid #2563EB; border-radius: 8px; color: #2563EB; font-size: 12px; font-weight: 700;">
                {{ session('error') }}
            </div>
        @endif
                                ...
                                <div style="display: flex; flex-wrap: wrap; gap: 6px;">
                                    @if($transaction->status === 'Pending')
                                        ...
                                        <form method="POST" action="{{ route('admin.transactions.reject', $transaction) }}" style="display: inline;">
                                            @csrf
                                            <button type="submit" style="padding: 6px 10px; border-radius: 8px; background: #2563EB; color: white; font-size: 11px; font-weight: 900; cursor: pointer; border: none;">Reject</button>
                                        </form>
                                    @endif
                                    ...
                                    <form method="POST" action="{{ route('admin.transactions.delete', $transaction) }}" style="display: inline;" onsubmit="return confirm('Delete this transaction? Balance may be adjusted.');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" style="padding: 6px 10px; border-radius: 8px; background: #2563EB; color: white; font-size: 11px; font-weight: 900; cursor: pointer; border: none;">Delete</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
    ...
    <div style="background: white; border-radius: 14px; padding: 24px; max-width: 400px; width: 90%;">
        ...
        <form method="POST" id="transactionForm">
            ...
            <div style="display: flex; gap: 10px;">
                ...
                <button type="button" onclick="closeTransactionModal()" style="flex: 1; height: 40px; border-radius: 8px; background: #2563EB; color: white; font-size: 12px; font-weight: 900; cursor: pointer; border: none;">
                    Cancel
                </button>
            </div>
        </form>
