        ...
        <form method="GET" action="{{ route('admin.users.transactions', $user) }}" class="p-4">
            <div style="display: flex; gap: 12px; align-items: end; flex-wrap: wrap;">
                ...
                @if(request('type') || request('status'))
                    <a href="{{ route('admin.users.transactions', $user) }}" style="height: 40px; padding: 0 20px; border-radius: 10px; background: #2563EB; color: white; font-size: 12px; font-weight: 900; cursor: pointer; display: inline-flex; align-items: center; text-decoration: none;">
                        Clear
                    </a>
                @endif
            </div>
