...
<div class="wrap">
    <div class="whitePanel" style="margin-bottom: 12px;">
        <form method="GET" action="{{ route('admin.inventory') }}" style="padding: 16px;">
            <div style="display: flex; gap: 12px; align-items: end;">
                ...
                @if(request('search'))
                    <a href="{{ route('admin.inventory') }}" style="height: 40px; padding: 0 20px; border-radius: 10px; background: #2563EB; color: white; font-size: 12px; font-weight: 900; cursor: pointer; display: inline-flex; align-items: center; text-decoration: none;">
                        Clear
                    </a>
                @endif
            </div>
    ...
    <div class="whitePanel">
        ...
        @if(session('error'))
            <div style="margin: 12px 14px; padding: 12px; background: #dbeafe; border: 1px solid #2563EB; border-radius: 8px; color: #1e40af; font-size: 12px; font-weight: 700;">
                {{ session('error') }}
            </div>
        @endif
                            ...
                            <td style="padding: 12px 14px;">
                                <div style="display: flex; gap: 6px;">
                                    ...
                                    <form method="POST" action="{{ route('admin.inventory.delete', $car) }}" style="display: inline;" onsubmit="return confirm('Are you sure you want to delete this vehicle?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" style="padding: 6px 12px; border-radius: 8px; background: #2563EB; color: white; font-size: 11px; font-weight: 900; cursor: pointer; border: none;">
                                            Delete
                                        </button>
                                    </form>
                                </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" style="padding: 40px; text-align: center; color: #6b7280; font-size: 13px;">
                                No vehicles found. <a href="{{ route('admin.inventory.create') }}" style="color: #2563EB; font-weight: 700; text-decoration: none;">Add your first vehicle</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
